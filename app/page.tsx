...
const SKILLS = [{name:"PHP",level:95,color:"#6366f1"},{name:"Laravel",level:92,color:"#ef4444"},{name:"Python",level:82,color:"#f59e0b"},{name:"Django",level:78,color:"#10b981"},{name:"React",level:75,color:"#3b82f6"},{name:"JavaScript",level:80,color:"#eab308"},{name:"MySQL",level:88,color:"#06b6d4"},{name:"Docker",level:70,color:"#8b5cf6"}];
...
const PROJECTS = [
  {name:"NexPulse",tagline:"AI-powered fraud protection for elderly users",problem:"Elderly users are disproportionately targeted by online scams. Existing platforms offer no real-time protection.",solution:"Built a full-stack social platform with a real-time ML pipeline that flags fraudulent messages and phishing links before they reach users.",result:"Reduced scam exposure with sub-100ms detection latency. Designed accessible UI with large typography and plain-language warnings.",tech:["Python","Django","React","Machine Learning","HTML5","CSS"],color:"#3b82f6"},
  {name:"TCP Client Server",tagline:"High-availability distributed networking system",problem:"Production systems need fault-tolerant networking with zero single-point-of-failure.",solution:"Engineered an async C-based TCP client-server using socket programming with active-passive clustering for load balancing.",result:"Achieved seamless failover with optimal load distribution across nodes using distributed clustering architecture.",tech:["C","TCP/IP","Linux","Socket Programming","Distributed Systems"],color:"#10b981"},
];
const LOOKING_FOR = [
  {icon:"💼",title:"Role type",value:"Full-time · Contract"},
  {icon:"🏢",title:"Company size",value:"Startup to Enterprise"},
  {icon:"📍",title:"Location",value:"Ontario · Remote · Hybrid"},
...
export default function Portfolio() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)"}}>
      {/* Ticker */}
      <div style={{background:"var(--accent)",overflow:"hidden",padding:"10px 0",borderBottom:"1px solid var(--border)"}}>
        <div style={{display:"flex",animation:"ticker 25s linear infinite",width:"max-content"}}>
          {tickerItems.map((item,i)=>(
            <span key={i} style={{color:"white",fontSize:"12px",fontWeight:500,padding:"0 32px",whiteSpace:"nowrap",letterSpacing:"0.5px"}}>{item} &nbsp;·</span>
          ))}
        </div>
      </div>

      {/* Nav */}
      <nav style={{position:"sticky",top:0,zIndex:100,background:"var(--bg)",borderBottom:"1px solid var(--border)",padding:"0 40px",display:"flex",alignItems:"center",justifyContent:"space-between",height:"64px",backdropFilter:"blur(12px)"}}>
        ...
        <div style={{display:"flex",gap:"32px",alignItems:"center"}}>
          ...
          <button onClick={()=>setDark(!dark)} style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:"100px",padding:"8px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:"8px",color:"var(--text-secondary)",fontSize:"13px",fontWeight:500}}>
      ...
      <main style={{maxWidth:"1000px",margin:"0 auto",padding:"0 40px 80px"}}>
        ...
        <section style={{padding:"80px 0 60px",display:"grid",gridTemplateColumns:"1fr auto",gap:"40px",alignItems:"center"}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"16px"}}>
              ...
              <span style={{fontSize:"13px",color:"var(--text-secondary)",fontWeight:500}}>Available for work</span>
            </div>
            <h1 style={{fontSize:"52px",fontWeight:800,color:"var(--text)",marginBottom:"16px",lineHeight:1.1,fontFamily:"Syne,sans-serif"}}>
              Hi, I'm Divyam.<br/><span style={{color:"var(--accent)"}}>I build things</span><br/>that scale.
            </h1>
            <p style={{fontSize:"16px",color:"var(--text-secondary)",maxWidth:"480px",marginBottom:"28px",lineHeight:1.7}}>
              Software developer with 5+ years of experience building backend systems and full-stack web applications. Based in Windsor, ON — open to Ontario and remote roles.
            </p>
            <div style={{display:"flex",gap:"12px",flexWrap:"wrap"}}>
              ...
              <a href="https://linkedin.com" target="_blank" style={{background:"var(--bg-card)",color:"var(--accent)",padding:"12px 14px",borderRadius:"10px",textDecoration:"none",border:"1px solid var(--border)",display:"flex",alignItems:"center"}}>
          ...
          <div style={{width:"160px",height:"160px",borderRadius:"50%",background:"var(--accent-light)",border:"4px solid var(--accent)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            ...
            {EXPERIENCE.map((job,i)=>(
              <div key={i} className="card" style={{padding:"28px"}}>
                ...
                <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
                  {job.tech.map(t=><span key={t} className="badge">{t}</span>)}
                </div>
              </div>
            ))}
          </div>
        ...
        <section id="contact">
          <div className="card" style={{padding:"48px",textAlign:"center",background:"var(--accent)",border:"none"}}>
            ...
            <p style={{color:"rgba(255,255,255,0.8)",fontSize:"15px",marginBottom:"32px"}}>Available immediately · Windsor, ON · Open to remote</p>
